...
if __name__ == "__main__":
    ...
    # 创建图形和轴
    fig, ax = plt.subplots()
    line, = ax.plot(fft_frequency, fft_result,linewidth=0.8)
    # 设置轴标签和标题
    ax.set_xlabel('Frequency (Hz)')
    ax.set_ylabel('Magnitude')
    ax.set_title('Frequency Domain Plot')
    # # 创建动画
    ani = FuncAnimation(fig, update, frames=np.arange(0, 10000), interval=1000/20,fargs=(line,))
    # 显示图形
    plt.show()
